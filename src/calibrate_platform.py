...
if real_robot:
        ...
        speed_limit = 0.1 #1
        speed_execution = 20 #50
        slow_speed_execution = 10 #30
...
def listener_to_pose(trans, rot):
        pose_result = Pose()
        pose_result.position.x = trans[0] 
        pose_result.position.y = trans[1] 
        pose_result.position.z = trans[2] 
        pose_result.orientation.x = rot[0] 
        pose_result.orientation.y = rot[1] 
        pose_result.orientation.z = rot[2] 
        pose_result.orientation.w = rot[3]
        return pose_result
...
def circular_trajectory(center, initial_pose, degree, rot_axis, center_waypoints, circle_waypoints, step = 2, rot_gripper = False): #R axis is x and Rot axis is z in its own rotation frame
        ...
        for deg in range(step, int(degree)+step, step):
                ...
                circle_waypoints.append(frame_to_pose(RM_world_own * new_waypoint_circle_own)) #pose
                center_waypoints.append(frame_to_pose(RM_world_own * new_waypoint_center_own)) #pose
...
def calibrate_callback(req): 
        ...
        z_offset = 0.05
        move_home()
        resp = CalibrationTriggerResponse()
        resp.success = False
        listener_success = False
        while not listener_success:
                try:
                        (trans,rot) = listener.lookupTransform('/torso_base_link', '/ID0001platform', rospy.Time(0))
                        listener_success = True
                except:
                       print("Cannot find the TFs. Is the robot node launched?")
                       rospy.sleep(0.5)
        pose_main_corner = listener_to_pose(trans,rot)
        print(pose_main_corner)

        if req.pos==1:
                corner_dist = [0,0,z_offset,0,0,0]
                rotate_gripperZ=False
                side="left"
        elif req.pos==2:
                corner_dist = [0,y_dist,z_offset,0,0,0]
                rotate_gripperZ=False
                side="left"                
        elif req.pos==3:
                corner_dist = [x_dist,0,z_offset,0,0,0]
                rotate_gripperZ=True
                side="right"
        elif req.pos==4:
                corner_dist = [x_dist,y_dist,z_offset,0,0,0]
                rotate_gripperZ=True
                side="right"
        else:
                print("The position must be: 1, 2, 3, or 4")
                return resp
        
        arm = motion_groups["arm_"+side]
        trash_pose = arm.get_current_pose().pose
        init_pose = arm.get_current_pose().pose
        waypoints_cal = [init_pose]
        pose_corner = get_shifted_pose(pose_main_corner, corner_dist)
        waypoints_cal.append(ATC1.correctPose(pose_corner, side, rotate = True, ATC_sign = -1, routing_app = rotate_gripperZ))
        plan, success = compute_cartesian_path_velocity_control([waypoints_cal], [speed_execution], arm_side=side)
        if success:
                execute_plan_async(("arm_"+side), plan) 
                resp.success = True
        return resp

rospy.Service('/calibrate_platform', CalibrationTrigger, calibrate_callback)
...
if __name__ == '__main__':
        ...
        gripper_end_frame_L.p = PyKDL.Vector(0, 0, 0.2356)#(0.0998, 0, 0.2356) #F/T sensor
        gripper_end_frame_L2 = PyKDL.Frame()
        gripper_end_frame_L2.p = PyKDL.Vector(0, 0, 0.25)
        gripper_end_frame_R = PyKDL.Frame() 
        gripper_end_frame_R.p = PyKDL.Vector(0, 0, 0.2536) #ATC
        gripper_end_frame_R2 = PyKDL.Frame() 
        gripper_end_frame_R2.p = PyKDL.Vector(0, 0, 0.27) #ATC
